__kernel void logicsimulation(uint length,
	...
	__global uint2 *belongs_to_robust_path_final_result)
{
	...
	for (int i = size-1; i>=0; i--)
	{
		...
		if (g)   // All other gates, if node is an input the else is executed, the gate type can also be used to determine the in_degree
		{
			...
			is_robust_or_non_robust_result[output_offset + c_node] = (uint2)(isRobustOrNonRobust_l, isRobustOrNonRobust_2);

		}
		else
		{
			//p1[output_offset + c_node] = 14; //boost::dynamic_bitset<>(generate4bit()).to_ulong();
			//p2[output_offset + c_node] = 3; //boost::dynamic_bitset<>(generate4bit()).to_ulong();
		}
	}
	...
	for (int i = size - 1; i >= 0; i--)
	{
		...
		if (g)
		{
			...
			if (out_edges[edges[c_node].x].x == c_node)
				btrofr = belongs_to_robust_path_final_result[output_offset + edges[c_node].x].x;
			else
				btrofr = belongs_to_robust_path_final_result[output_offset + edges[c_node].x].y;

			temp_final_result |= is_robust_path_to_po[output_offset + edges[c_node].x] & robust_result[output_offset + c_node].x & btrofr;

			/* robust_result[output_offset + c_node].second; */
			if (out_edges[edges[c_node].y].x == c_node)
				btrofr = belongs_to_robust_path_final_result[output_offset + edges[c_node].y].x;
			else
				btrofr = belongs_to_robust_path_final_result[output_offset + edges[c_node].y].y;

			temp_final_result |= is_robust_path_to_po[output_offset + edges[c_node].y] & robust_result[output_offset + c_node].y & btrofr;
			...
			if ((out_edges[c_node].x != 0) && (out_edges[c_node].y != 0))
			{
				...
				belongs_to_robust_path_final_result[output_offset + c_node] = (uint2)(result1,result2);
		...
		{
			belongs_to_robust_path_final_result[output_offset + c_node].x = is_robust_path_to_po[output_offset + c_node];
			belongs_to_robust_path_final_result[output_offset + c_node].y = is_robust_path_to_po[output_offset + c_node];
		}
	}
}