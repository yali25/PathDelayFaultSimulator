__kernel void logicsimulation(ulong length,
...
#pragma unroll 8
		for (int i = size-1; i >=0 ; i--)
		{
			...
			//if (out_edges[c_node]) 
			{
				...
				r1 = (edges[out_edges[c_node].x].x == c_node)?robust_result1[output_offset + out_edges[c_node].x]:robust_result2[output_offset + out_edges[c_node].x];
				path_po = is_robust_path_to_po[output_offset + out_edges[c_node].x];
				result |= (r1 & path_po);
				...
				r1 = (edges[out_edges[c_node].y].x == c_node)?robust_result1[output_offset + out_edges[c_node].y]:robust_result2[output_offset + out_edges[c_node].y];
				path_po = is_robust_path_to_po[output_offset + out_edges[c_node].y];
				result |= (r1 & path_po);
		...
		for (int i = 0; i < size; i++)
		{
			...
			if (g)
			{
				...
				if (out_edges[edges[c_node].x].x == c_node)
					btrofr = belongs_to_robust_path_final_result1[output_offset + edges[c_node].x];
				else
					btrofr = belongs_to_robust_path_final_result2[output_offset + edges[c_node].x];
				...
				if (out_edges[edges[c_node].y].x == c_node)
					btrofr = belongs_to_robust_path_final_result1[output_offset + edges[c_node].y];
				else
					btrofr = belongs_to_robust_path_final_result2[output_offset + edges[c_node].y];
	...
	)
{
	   ...
	   unsigned int  result1 = 0;
		...
		for (int i = 0; i < size; i++)
		{
			const unsigned int c_node = top_order[i];
			result1 = 0;
			result2 = 0;
			const unsigned int g = gate_type[c_node];
			if (g)   // All other gates, if node is an input the else is executed, the gate type can also be used to determine the in_degree
		...
		for (int i = 0; i < size; i++)
		{
			const unsigned int c_node = reverse_top_order[i];
			if ((out_edges[c_node].x != 0) && (out_edges[c_node].y != 0)) // Node is not an output
			{
				unsigned int result = 0;
	...
	__kernel void logicsimulation_opordered(ulong length,
	__global uint  *top_order,
	__global uint  *reverse_top_order,
	__global uint  *gate_type,
	__global uint2 *edges,
	...
	)
{
	...
	__kernel void logicsimulation_mem_opt(ulong length,
	__global uint  *top_order,
	__global uint  *reverse_top_order,
	__global uint  *gate_type,
	__global uint2 *edges,
       ...
       unsigned int  result2 = 0;
		...
		for (int i = 0; i < size; i++)
		{
			...
			if (g)
			{
				...
				btrofr = (out_edges[edges[c_node].x].x == c_node)?p1[output_offset + edges[c_node].x]:p2[output_offset + edges[c_node].x];
				temp_final_result |= is_robust_path_to_po[output_offset + edges[c_node].x] & robust_result1[output_offset + c_node] & btrofr;

				btrofr = (out_edges[edges[c_node].y].x == c_node)?p1[output_offset + edges[c_node].y]:p2[output_offset + edges[c_node].y];
				temp_final_result |= is_robust_path_to_po[output_offset + edges[c_node].y] & robust_result2[output_offset + c_node] & btrofr;
				...
				if ((out_edges[c_node].x != 0) && (out_edges[c_node].y != 0))
				{
					...
					t = (edges[out_edges[c_node].x].x == c_node)? robust_result1[output_offset + out_edges[c_node].x] : robust_result2[output_offset + out_edges[c_node].x];
					result1 = t & temp_final_result & is_robust_path_to_po[output_offset + out_edges[c_node].x];

					t = (edges[out_edges[c_node].y].x == c_node)? robust_result1[output_offset + out_edges[c_node].y] : robust_result2[output_offset + out_edges[c_node].y];
					result2 = t & temp_final_result & is_robust_path_to_po[output_offset + out_edges[c_node].y];
