...
#pragma unroll 8
  for (int i = 0; i < size; i++) {
    ...
    //	else
    //	{
    // p1[output_offset + c_node] =
    // boost::dynamic_bitset<>(generate4bit()).to_ulong(); p2[output_offset +
    // c_node] = boost::dynamic_bitset<>(generate4bit()).to_ulong();
    //	}
  }
  ...
  for (int i = size - 1; i >= 0; i--) {
    ...
    // if (out_edges[c_node])
    {
      ...
      /*if (edges[out_edges[c_node].x].x == c_node)
      {
              ...
              path_po = is_robust_path_to_po[output_offset +
      out_edges[c_node].x];
      }
      else
      {
  ...
  for (int i = 0; i < size; i++) {
    ...
    if (g) {
      ...
      if (out_edges[edges[c_node].x].x == c_node)
        btrofr = belongs_to_robust_path_final_result1[output_offset +
                                                      edges[c_node].x];
      else
        btrofr = belongs_to_robust_path_final_result2[output_offset +
                                                      edges[c_node].x];
...
__kernel void logicsimulation(ulong length, __global uint *top_order,
                              __global uint *reverse_top_order,
                              __global uint *gate_type, __global uint2 *edges,
...
__kernel void logicsimulation_opordered(
    ulong length, __global uint *top_order, __global uint *reverse_top_order,
    __global uint *gate_type, __global uint2 *edges, __global uint2 *out_edges,
    __global uint *p1, __global uint *p2, __global uint *robust_result1,
    __global uint *robust_result2,
    __global uint *is_robust_or_non_robust_result1,
    __global uint *is_robust_or_non_robust_result2,
    __global uint *is_robust_path_to_po) {
...
__kernel void logicsimulation_mem_opt(
    ulong length, __global uint *top_order, __global uint *reverse_top_order,
    __global uint *gate_type, __global uint2 *edges, __global uint2 *out_edges,
    __global uint *p1, __global uint *p2, __global uint *robust_result1,
    __global uint *robust_result2,
  ...
  for (int i = 0; i < size; i++) {
    ...
    if (g) {
      ...
      btrofr = (out_edges[edges[c_node].x].x == c_node)
                   ? p1[output_offset + edges[c_node].x]
                   : p2[output_offset + edges[c_node].x];
      temp_final_result |=
          is_robust_path_to_po[output_offset + edges[c_node].x] &
          robust_result1[output_offset + c_node] & btrofr;

      btrofr = (out_edges[edges[c_node].y].x == c_node)
                   ? p1[output_offset + edges[c_node].y]
                   : p2[output_offset + edges[c_node].y];
      temp_final_result |=
          is_robust_path_to_po[output_offset + edges[c_node].y] &
          robust_result2[output_offset + c_node] & btrofr;
