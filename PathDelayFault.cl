	__kernel void logicsimulationv1(ulong length,
       ...
       unsigned int  result2 = 0;
		...
		for (int i = 0; i < SIZE; i++)
		{
			...
			if (g)
			{
				...
				btrofr = (out_edges[edges[c_node].x].x == c_node)?belongs_to_robust_path_final_result1[output_offset + edges[c_node].x]:belongs_to_robust_path_final_result2[output_offset + edges[c_node].x];
				temp_final_result |= is_robust_path_to_po[output_offset + edges[c_node].x] & robust_result1[output_offset + c_node] & btrofr;

				btrofr = (out_edges[edges[c_node].y].x == c_node)?belongs_to_robust_path_final_result1[output_offset + edges[c_node].y]:belongs_to_robust_path_final_result2[output_offset + edges[c_node].y];
...
__kernel void logicsimulationv2(
	__global uint  *gate_type,
	__global uint2 *edges,
	__global uint2 *out_edges,
	__global uint  *p1,
